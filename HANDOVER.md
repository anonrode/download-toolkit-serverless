...
**Latest release on GitHub:** `AnonDownloader-v3.1.1-{arm64-v8a,armeabi-v7a,universal,x8,x86_64}.apk` (tag `v3.1.1`, gradle versionCode 311, published 2026-09-13T18:09Z from `7f66249`) — the first release ever compiled + tested from the full arc. **KNOWN-BAD: every download crashes on-device** (NameSanitizer `<clinit>` regex embedded-flag throw, `10a6958` hotfix, not yet released). The old `v3.1.0` artifacts remain below it as pre-compiler-era history (that APK was the last one devices could have installed before today; its code had never passed a compiler since 63a7032 until CI did it on the arc).
